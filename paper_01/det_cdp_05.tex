...
% EJC papers must begin with the two lines above.
% Additional packages used by the manuscript.
\usepackage{mathtools}
\usepackage{enumitem}

% Small line-breaking allowance to avoid overfull boxes in the E-JC layout.
\emergencystretch=2em

% The theorem-like environments theorem, lemma, corollary, proposition,
% definition, and remark are supplied by e-jc.sty.
...
\MSC{05A05, 05C70, 68Q25, 37B15}

\title{On the Maximum Duration of a Deterministic\\ Card-Dueling Process\\[4pt]
\large A Combinatorial Analysis}
\author{Ali Nehrani\\ \address{GROUND Inc., Tokyo, Japan\\
\url{https://en.groundinc.co.jp/} }\\ \email{nehrani@gmail.com}}
\begin{document}
% \dateline{Month Day, Year}{Month Day, Year}{Month Day, Year}
\maketitle
...
\end{lemma}
\begin{proof}
Within a phase only $\WA$, $\WB$, and $\NN$ occur. In a $\WA$ duel the $A$-top card stays fixed while the $B$-top card moves to the bottom; in a $\WB$ duel the symmetric operation occurs; and in an $\NN$ duel both top cards move to the bottoms of their decks. Thus each deck is either left unchanged or cyclically rotated by one position. In either case no card overtakes another, so the relative cyclic order in each deck is invariant throughout the phase.
\end{proof}
...
We now construct, for each $n$, an explicit outcome table and initial configuration whose duration attains the upper bound. The idea is to make each phase perform a nearly complete raster scan of the active torus. Most cells in a row are $\WA$ cells, so the trajectory moves horizontally through that row by rotating only the $B$-deck. One carefully placed $\NN$ cell then advances both decks and drops the trajectory to the next row. The realized tie is delayed until the last row, first column; the remaining tie cells are exactly the future matching ties that the upper-bound argument says must be avoided.

The proof has four moving parts. Lemma \ref{lem:restrict} shows that the global table restricts to the same local staircase table on every active layer. Lemmas \ref{lem:row-dynamics}--\ref{lem:row-cardinality} and Theorem \ref{thm:local-staircase} prove that this local table has the desired raster-scan trajectory. Lemma \ref{lem:small-local} checks the two exceptional tiny phases. Finally, Lemma \ref{lem:phase-reset} proves that the terminal tie leaves the physical decks in the canonical order needed for the next smaller phase. This last reset lemma is what turns a one-phase construction into a full $n$-phase game.
...
For example, when $n=4$ the table is
\[
\begin{array}{c|cccc}
 & b_0 & b_1 & b_2 & b_3\\
\hline
...
\end{array}
\]
The first phase starts at $(a_0,b_0)$. Each $\WA$ moves right in the displayed table, each $\NN$ moves diagonally down-right to the next row, and the terminal tie is delayed until $(a_3,b_0)$. The other three ties on the anti-diagonal are precisely the tie cells reserved for later phases.
...
As a concrete coordinate example, take $n=3$. At phase size $m=2$ the canonical active decks are
\[
A_2=[a_0,a_1],\qquad B_2=[b_1,b_2].
\]
The local column $v=0$ is therefore the global card $b_1$, while $v=1$ is the global card $b_2$. Thus the local cell $(1,0)$ represents the global pair $(a_1,b_1)$, and the local cell $(0,1)$ represents $(a_0,b_2)$. This example is the reason for the translation $j=n-m+v$ rather than simply $j=v$ in the restriction lemma.
...
\begin{lemma}[Accumulated row offset]\label{lem:row-offset}
For $m\ge2$, immediately before the terminal tie of the staircase phase, the local row coordinate is $m-1$. Equivalently, the $A$-deck has been cyclically rotated exactly $m-1$ times relative to its initial order, so its top-to-bottom order is
\[
[a_{m-1},a_0,\dots,a_{m-2}].
\]
\end{lemma}
\begin{proof}
The $A$-deck rotates exactly on $\NN$ moves in this construction. For $m>2$, Lemma \ref{lem:row-dynamics} gives one $\NN$ exit from each intermediate row $u=0,\dots,m-2$, hence $m-1$ total $\NN$ moves before the terminal tie. For $m=2$, the explicit trajectory in Lemma \ref{lem:small-local} has one $\NN$ move, again $m-1$. A cyclic rotation by $m-1$ positions sends $[a_0,\dots,a_{m-1}]$ to $[a_{m-1},a_0,\dots,a_{m-2}]$.
\end{proof}
...
It remains only to check the order of the remaining decks. By Lemma \ref{lem:row-offset}, immediately before the terminal tie the $A$-deck is
\[
[a_{m-1},a_0,\dots,a_{m-2}].
\]
The terminal local column is $0$, so the $B$-deck has top card $b_{n-m}$; cyclic order preservation gives
\[
...
The extremal witness is deliberately asymmetric: it uses $\WA$, $\NN$, and $\TT$ entries but no $\WB$ entries. This is not a defect of the lower bound, since a single attaining construction suffices to prove sharpness. It does, however, leave a structural classification question: are all extremizers asymmetric after relabeling, or can one impose natural balance or symmetry constraints on the outcome table and still attain the same duration?
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{Balanced or symmetric extremizers.} Determine the maximum duration under additional constraints such as requiring both $\WA$ and $\WB$ to appear, requiring a specified number of each win type, or imposing a symmetry relation between the two decks.
