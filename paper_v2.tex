...
\subsection{Phase-1 traversal}
Row $i$ ($0 \le i \le n - 2$) is entered at column $j_{\mathrm{start}} \equiv -i \pmod n$. The path scans rightward along
$\WA$-cells until it reaches $c_N(i) = -2 - i$, where the single $\NN$ outcome sends it diagonally to the next row.

\begin{lemma}[Row handoff]\label{lem:handoff}
For $0 \le i \le n-2$, the $\NN$ transition at the end of row $i$ maps $(i, -2-i) \mapsto (i + 1, -1 - i)$. Since
$-1 - i \equiv -(i + 1) \pmod n$, the landing column equals the entry column $j_{\mathrm{start}} = -(i+1)$ of row
$i+1$, and the forbidden cell $(i, n-1-i) = (i, -1-i)$ is skipped. Consequently row $i$ visits exactly $n - 1$ columns.
\end{lemma}
\begin{proof}
By~\eqref{eq:trans} the $\NN$ outcome at $(i, -2 - i)$ yields $(i + 1, -1 - i)$. The identity $-1 - i \equiv -(i + 1)$ is
immediate, and $-(i + 1)$ is the entry column of row $i+1$. The forbidden cell of row $i$ sits at column $-1 - i$, which
the scan jumps over; hence row $i$ visits every column except $-1 - i$.
\end{proof}

The final row $i = n - 1$ is entered at $j = 1$ and scans all $n$ columns $1, 2, \dots, n - 1, 0$, ending in the active
tie at $j = 0$. Hence
\begin{equation}\label{eq:L1}
L_1 = \underbrace{(n - 1)(n - 1)}_{\text{first } n-1 \text{ rows}} + \underbrace{n}_{\text{final row}}
= n^2 - n + 1 = m_1^2 - m_1 + 1.
\end{equation}

\subsection{Residue arithmetic at the phase boundary}
\paragraph{Deck $A$.} A card of $A$ moves to the bottom only on $\WB$ or $\NN$. Phase 1 uses no $\WB$ and exactly one
$\NN$ per row for the first $n - 1$ rows, so $A$ undergoes $n - 1$ rotations:
$[a_0, \dots, a_{n-1}] \mapsto [a_{n-1}, a_0, \dots, a_{n-2}]$. The top card $a_{n-1}$ is removed by the active tie,
leaving $D^{(n-1)}_A = [a_0, \dots, a_{n-2}]$.

\paragraph{Deck $B$.} A card of $B$ moves to the bottom on $\WA$ or $\NN$, i.e.\ at every non-tie step, of which there
are $n^2 - n$. Since $n^2 - n \equiv 0 \pmod n$, deck $B$ returns to $[b_0, \dots, b_{n-1}]$ at the tie. The top card
$b_0$ is removed, leaving $[b_1, \dots, b_{n-1}]$, i.e.\ $[b'_0, \dots, b'_{n-2}]$ under $b'_k := b_{k+1}$.
